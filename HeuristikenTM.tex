...
Das vorliegende Kapitel beschreibt einen Ansatz zur Lösung des \TM mithilfe des Ressource Sharing Algorithmus. Der Im Folgenden vorgestellte Lösungsansatz und Algorithmus orientiert sich an der Lösung des Gate Sizing Problems mit Ressource Sharing. Ausführliche Informatinonen hierzu befinden sich in \cite{GateSizing}.\\
...
Sei $G$ ein Circuit ohne teilweise überflüssige Subcircuits. Dieser Circuit bildet das einzige  Element in der Kundenmenge $\mathcal{C}$. Das Optimierungsprblem lässt sich nun wiefolgt darstellen:
\[ \]
\textcolor{red}{timing graph mit reinbringen damit klar ist was das delay ist / wie es berechnet wird inkl. passenden Bild}
Diese Formulierung ist äuivalent zu: 

\[\]
Im Folgenden wird mittels der zweiten Formulierung optimiert. Dies erscheint, da die Formulierung komplexer ist, nicht rational, bringt im späteren Verlauf jedoch einen deutlichen Vorteil gegenüber der ersten Formulierung.\\
...
Das Optimierungsproblem lässt sich nun wiefolgt darstellen:
\[ \]


Es bleibt der Blocklöser für $\mathcal{B}_G$ ordnet nun jedem Circuit-Kandidaten von $G$ den entsprechenden Ressourcenverbrauch mit $Power(G)$ und  \\ $Delay(P) \ \forall \text{inklusionsmaximalen Wege } P$ zu. \\
